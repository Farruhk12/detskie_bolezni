...
// Отправка данных в GAS через серверный прокси (обход CORS)
...
async function submitST() {
  ...
  try {
    ...
    if (j.ok) {
      showSTResults(j.grades, j.avgGrade, answers);
      gasPost('saveExamResult', {
        studentId: sid,
        course:    USER_PREFS ? (USER_PREFS.course  || '') : '',
        faculty:   USER_PREFS ? (USER_PREFS.faculty || '') : '',
        answers:   answers,
        grades:    j.grades,
        avgGrade:  j.avgGrade
      });
...
document.getElementById('stBackToTopics').addEventListener('click', ()=>switchView('topics'));
...
async function submitEF() {
  ...
  try {
    ...
    if (j.ok) {
      showEFResults(j.grades, j.avgGrade, answers);
      gasPost('saveExamResult', {
        studentId: sid,
        course:    USER_PREFS ? (USER_PREFS.course  || '') : '',
        faculty:   USER_PREFS ? (USER_PREFS.faculty || '') : '',
...
document.getElementById('efBackToTopics').addEventListener('click', ()=>switchView('topics'));
